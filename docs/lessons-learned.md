...
- Don't reverse-engineer an undocumented API by trial-and-error — read the vendor's own client. The DS2 selective-download commit was guessed three times (`BT.File set`+`Complete` with `list_id`; then `Task.List.Polling/download` with `file_indexes`+`destination`) and each shipped/probed wrong — tasks 404'd or were created but hung at the start. The authoritative spec is the **DSM web UI's own JavaScript on the NAS** (`/var/packages/DownloadStation/target/ui/download.js`, readable over SSH). It shows the real flow plainly: create `create_list=true` → `list_id`; on OK `SYNO.DownloadStation2.Task.Complete` `start` v1 `{id: list_id}` (param is **`id`**) starts the download to the list's own destination and returns a `task_id`; a subset is just unwanted files marked `wanted:false` via `BT.File` `set` `{task_id, index, wanted:false}` on the completed task (selection is the `wanted` flag, NOT `file_indexes`). For any undocumented Synology API, grep the app's `download.js`/`ui` bundle before probing — the exact api/method/version/params are right there.
