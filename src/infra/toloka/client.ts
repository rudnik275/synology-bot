...
export class TolokaClient {
  ...
  async search(query: string): Promise<TolokaResult[]> {
    const html = await this.getWithAuth(
      `${this.config.baseUrl}/tracker.php?nm=${encodeURIComponent(query)}`
    )
  ...
  private async getWithAuth(url: string): Promise<string> {
  ...
  private async fetchWithAuth(url: string): Promise<Response> {
    return fetch(url, {
      headers: {
        ...
        'User-Agent': 'Mozilla/5.0 (compatible; nas-torrent-bot)',
      },
      redirect: 'manual',
    })
  }
