...
- Don't ship an unverified API flow as if confirmed. The selective-download commit had been coded as `BT.File set` + `Complete start` (param `list_id`), guessed from read-only probing and shipped behind a "best-faithful, not exercised" comment. On the first real write it failed: `BT.File` 404s on a list_id and `Complete` wants a param named `id`. The actual commit is a single `SYNO.DownloadStation2.Task.List.Polling` `download` (`list_id` + `file_indexes` comma-separated + `destination`, which is required again). A comment admitting "not exercised" is a deferred bug, not a safeguard — write-test the path or guard it behind a fallback before release.
